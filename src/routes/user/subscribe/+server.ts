...
import { customer as customerTable, user as userTable, subscription as subscriptionTable } from '$lib/models/schema';
...
export const POST: RequestHandler = async ({ locals, request, getClientAddress }) => {
    ...
    if (locals.user.customer !== null) {
        await db.transaction(async (tx) => {
            ...
            await tx.delete(customerTable).where(eq(customerTable.braintreeCustomerId, locals.user!.customer!));
        })
    }
    ...
    try {
        ...
        error(500, "You were charged but something went wrong on our end. If you don't have access to your tier, please contact support. Don't attempt to checkout again.")
    }
...
const createCustomer = async (request: Request, requestData: any, user: User, getClientAddress: () => string) => {
    ...
    const createCustomerResult = await BraintreeGateway.customer.create({
        ...
        riskData: {
            ...
            customerBrowser: userAgent !== null ? userAgent.length <= 255 ? userAgent : undefined : undefined
        },
        creditCard: {
            billingAddress: {
    ...
    if (!createCustomerResult.success) {
        ...
        if ((createCustomerResult as any).verification?.status !== undefined && (createCustomerResult as any).verification.status !== "verified") {
            error(400, "There was a problem processing your payment information; please double check your payment information and try again");
        }

        throw { reason: "Weird error creating customer", error: createCustomerResult };
    }

    if (createCustomerResult.customer.paymentMethods === undefined || createCustomerResult.customer.paymentMethods[0] === undefined) {
...
const createSubscription = async (planId: string, user: User, customer: braintree.Customer) => {
    ...
    if (!createSubscriptionResult.success) {
        if (createSubscriptionResult.errors.deepErrors().length !== 0) {
            throw { reason: "Validation error on creating subscription", error: createSubscriptionResult.errors };
        }

        if ((createSubscriptionResult as any).transaction?.status !== undefined && (createSubscriptionResult as any).transaction.status !== "settled" && (createSubscriptionResult as any).transaction.status !== "settling" && (createSubscriptionResult as any).transaction.status !== "authorizing" && (createSubscriptionResult as any).transaction.status !== "authorized" && (createSubscriptionResult as any).transaction.status !== "settlement_confirmed" && (createSubscriptionResult as any).transaction.status !== "submitted_for_settlement" && (createSubscriptionResult as any).transaction.status !== "settlement_pending") {
            error(400, "There was a problem processing your payment information; please double check your payment information and try again");
        }
