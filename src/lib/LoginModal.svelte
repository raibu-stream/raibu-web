...
{#if loginModalOn}
	<Modal
		title={forgetPassword ? 'Reset your password' : 'Log in'}
		closeModal={() => (loginModalOn = false)}
	>
		{#if !forgetPassword}
			<form
					...
					{:else}
						...
						>
							...
							<small class="text-xs text-neutral-200">
								...
								noreply@raibu.stream. If you still don't see it, your email address may not be
								verified.
							</small>
						</div>
					{/if}
