...
export const incrementOrCreateTimeout = async (id: string, expires: Date = createDate(new TimeSpan(1, "m"))) => {
    ...
    if (timeout !== undefined) {
        ...
        return (await db
            ...
            .where(eq(timeOut.timerId, id)).returning())[0];
    } else {
        return (await db.insert(timeOut).values({
            timerId: id,
            expires,
            attempts: 1
        }).returning())[0];
