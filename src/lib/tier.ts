...
export const zTier = z
    .object({
        maxConcurrentStreams: z.union([z.literal(1), z.literal(2), z.literal(3), z.literal(6)]),
        maxConcurrentViewers: z.union([z.literal(80), z.literal(125), z.literal(200), z.literal(300), z.literal(500), z.literal(800)]),
        maxBitrateInKbps: z.union([z.literal(3000), z.literal(6000), z.literal(8000)])
    });

export const getPreset = (tier: Tier) => {
    if (tier.maxConcurrentStreams === 1 && tier.maxConcurrentViewers === 80 && tier.maxBitrateInKbps == 3000) {
        return "Party Starter";
    } else if (tier.maxConcurrentStreams === 3 && tier.maxConcurrentViewers === 300 && tier.maxBitrateInKbps == 6000) {
        return "Tier2"
    } else if (tier.maxConcurrentStreams === 6 && tier.maxConcurrentViewers === 800 && tier.maxBitrateInKbps == 8000) {
        return "Tier3"
    }
}

export const getPricing = (tier: Tier): {
    streamsCost: number,
    viewersCost: number,
    bitrateCost: number,
    ...
    return {
        ...
        viewersCost,
        bitrateCost,
        discountedTotal: preset === undefined || preset === "Party Starter" ? undefined : total * 0.90,
        total
    }
}

export const tierFromIndexs = (maxStreamsIndex: number, maxViewersIndex: number, maxBitrateIndex: number): Tier => {
    return {
        maxBitrateInKbps: maxBitrateIndex === 0
            ? 3000
            : maxBitrateIndex === 1
                ? 6000
                : maxBitrateIndex === 2
